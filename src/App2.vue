<template>
  <div class="scene flex-column justify-center">
  ...
  nextTick(() => {
    ...
    function render() {
      ...
      if (!LEFT_WALL_BB(left_wall, ballIntersect) &&
        ...
        if (PROJECT2_BB(project2, ballIntersect) && !project2Flag.value) {
          import("./components/Project2.vue").then((res) => {
            component.value = res?.default;
            project2Flag.value = flag.value = true
            dialog.value = true
            project2.removeFromParent()
          })
        }

        if (PROJECT3_BB(project3, ballIntersect) && !project3Flag.value) {
          import("./components/Project3.vue").then((res) => {
            component.value = res?.default;
            project3Flag.value = flag.value = true
            dialog.value = true
            project3.removeFromParent()
          })
        }

        if (PROJECT4_BB(project4, ballIntersect) && !project4Flag.value) {
          import("./components/Project4.vue").then((res) => {
            component.value = res?.default;
            project4Flag.value = flag.value = true
            dialog.value = true
            project4.removeFromParent()
          })
        }

        if (PROJECT5_BB(project5, ballIntersect) && !project5Flag.value) {
        ...
        if (GOAL_BB(goal, ballIntersect) && !goalFlag.value) {
          import("./components/Goal.vue").then((res) => {
            component.value = res?.default;
            goalFlag.value = flag.value = true
            dialog.value = true
          })
        }
